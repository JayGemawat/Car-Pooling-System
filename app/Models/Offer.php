...
class Offer
{
    public static function getUpcoming(): array
    {
        $pdo  = getDB();
        $stmt = $pdo->prepare(
            'SELECT id, "from", "to", uptime, vehicle, distance_km
             FROM offers
             WHERE uptime > NOW() AND status != \'archived\'
             ORDER BY uptime ASC',
        );
        $stmt->execute();
    ...
    public static function search(string $from, string $to, string $uptime, string $downtime): array
    {
        ...
        $stmt = $pdo->prepare(
            'SELECT r1.cid FROM route r1
             ...
             WHERE LOWER(r1.place) = LOWER(?) AND LOWER(r2.place) = LOWER(?) AND r1.serialno < r2.serialno',
        );
        ...
        $stmt2 = $pdo->prepare(
            "SELECT id, vehicle, \"from\", \"to\", uptime, distance_km FROM offers
    ...
    public static function create(array $data): int
    {
        ...
        // Auto-calculate distance asynchronously (best-effort)
        $distanceKm = null;
        if (!empty($data['from']) && !empty($data['to'])) {
            $distanceKm = self::distanceBetween($data['from'], $data['to']);
        }

        $stmt = $pdo->prepare(
            'INSERT INTO offers (uid, "from", "to", uptime, people, price, vehicle, description, distance_km)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
             RETURNING id',
        );
        $stmt->execute([
     ...
     * Searches globally (not restricted to India) to support local areas.
     */
    public static function geocodeServer(string $place): ?array
    {
        ...
        $ctx = stream_context_create(['http' => [
            'header'  => "User-Agent: JaanaHai/1.0\r\n",
            'timeout' => 5,
        ]]);
